...
/* 리모컨을 누를 때만 동작하게 해봅시다.
  리모컨의 같은 버튼을 계속 누르면 FFFFFFFF의 신호가 전송됩니다.
  이를 이용하여 FFFFFFFF의 신호를 받으면 계속 반복하게 코드를 작성해봅시다. */
...
void loop() {
  if (irrecv.decode(&results)) { //적외선 신호를 해석한 경우
    switch (results.value) {
      case 0xFF30CF: leftforward(); val = 1; break; //1을 누르면 좌향전진
      case 0xFF18E7: forward(); val = 2; break; //2번을 누르면 전진
      ...
      case 0xFFFFFFFF: vals(val); break;
    }
